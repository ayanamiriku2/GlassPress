...
<form method="post" action="<?= $adminUrl ?>/pages<?= $isEdit ? "/update/{$p['id']}" : '/store' ?>" id="page-editor-form" class="editor-layout">
        ...
        <div class="glass-card" id="seo-panel">
            ...
            <div id="seo-fields" class="<?= empty($seoMeta) ? 'hidden' : '' ?>">
                ...
                <div class="form-group">
                    ...
                    <textarea name="seo_description" rows="2" class="form-input" maxlength="160"><?= htmlspecialchars($seoMeta['seo_description'] ?? '') ?></textarea>
                </div>
                <div class="form-group">
        ...
        <div class="glass-card">
            ...
            <div class="form-group">
                ...
                <select name="parent_id" class="form-input">
                    ...
                    <option value="<?= $pp['id'] ?>" <?= ($p['parent_id'] ?? 0) == $pp['id'] ? 'selected' : '' ?>>
            ...
            <div class="form-group">
                ...
                <select name="page_template" class="form-input">
                    <option value="default" <?= ($p['page_template'] ?? 'default') === 'default' ? 'selected' : '' ?>>Default</option>
                    <option value="full-width" <?= ($p['page_template'] ?? '') === 'full-width' ? 'selected' : '' ?>>Full Width</option>
                    <option value="sidebar-left" <?= ($p['page_template'] ?? '') === 'sidebar-left' ? 'selected' : '' ?>>Sidebar Left</option>
                    <option value="landing" <?= ($p['page_template'] ?? '') === 'landing' ? 'selected' : '' ?>>Landing Page</option>
                </select>
            </div>
            <div class="form-group">
                <label>Order</label>
                <input type="number" name="menu_order" class="form-input" value="<?= (int) ($p['menu_order'] ?? 0) ?>" min="0">
            </div>
        </div>
        ...
        <div class="glass-card">
            ...
            <label class="checkbox-label">
                <input type="checkbox" name="comment_status" value="open" 
                       <?= ($p['comment_status'] ?? 'closed') === 'open' ? 'checked' : '' ?>>
                Allow comments
            </label>
        </div>
    </div>
...
(function() {
    ...
    document.getElementById('page-editor-form').addEventListener('submit', function() {
        if (!isHtmlMode) html.value = visual.innerHTML;
    });
...
.editor-title-input { width: 100%; border: none; background: transparent; font-size: 24px; font-weight: 600; color: var(--text); outline: none; padding: 8px 0; }
.editor-slug-wrap { padding: 8px 20px 16px; font-size: 13px; color: var(--text-muted); display: flex; align-items: center; gap: 4px; border-bottom: 1px solid var(--glass-border); }
.slug-base { opacity: .6; }
.editor-slug-input { border: 1px solid var(--glass-border); background: var(--glass-bg); color: var(--text); padding: 2px 8px; border-radius: 4px; font-size: 13px; flex: 1; }
.editor-toolbar { display: flex; gap: 2px; padding: 8px 12px; border-bottom: 1px solid var(--glass-border); flex-wrap: wrap; }
.editor-btn { background: transparent; border: 1px solid transparent; color: var(--text); padding: 4px 10px; border-radius: 4px; cursor: pointer; font-size: 14px; }
...
.editor-content { min-height: 400px; padding: 20px; font-size: 16px; line-height: 1.7; color: var(--text); outline: none; }
.editor-textarea { width: 100%; border: none; background: transparent; font-family: monospace; font-size: 14px; resize: vertical; }
.form-meta { display: flex; flex-direction: column; gap: 4px; font-size: 12px; color: var(--text-muted); padding: 8px 0; border-top: 1px solid var(--glass-border); margin: 8px 0; }
